...
Most QA tasks from the LLM era seem to have two phases:
...
make it a promising choice for knowledge representation.
...
I'm currently running a local LLM on llama-cpp for generating 
the TypeQL for both KG construction and question-answering .
At the time of writing, this was `TheBloke/deepseek-coder-33B-instruct-GGUF:Q4_K_M` from huggingface. The prompts used can be found in the `prompts` folder.
